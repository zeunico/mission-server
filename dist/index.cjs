  ...
  try {
    ...
    if (roomId) {
      ...
      for (const userId of participants) {
        ...
        const isInEtat = await service2.etatByUser(missionId, userObjectId);
        if (Object.values(etat_enum_default).includes(isInEtat)) {
          ...
          const inscription = await service2.inscriptionMission(missionId, userObjectId);
          if (inscription) {
            results.push({
              userId,
...
var limit = (0, import_p_limit.default)(10);
var addConnectedUsersToMission = /* @__PURE__ */ __name(async () => {
  try {
    const rooms = await RoomService.findAll();
    for (const room of rooms) {
      const missions = await missionService2.findByRoomId(room._id);
      for (const mission of missions) {
        const missionId = mission._id;
        return limit(() => axios4.post(`${config2.BASE_URL}/mission/${missionId}/inscrireRoom/`));
      }
    }
  } catch (error) {
